...
// handle sending a message
  ...
  return selectedUser? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            ...
            {messages.map((msg,index)=>(
                <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !==authUser._id && 'flex-row-reverse'}`}>
                    ...
                    <div className='text-center text-x5'>
                        ...
                        <p className='text-gray-500  md:text-sm'>{formatMessageTime(msg.createdAt)}</p>
                    </div>
                </div>
            ))}
            <div ref={scrollEnd}>
        ...
        <div className='flex-1 flex items-center justify-between bg-gray-100/12 px-3 rounded-full'>
            ...
            <label htmlFor="image">
                <img src={assets.gallery_icon} alt="" className='w-5 mr-1 cursor-pointer' />
            </label>
            </div>
        </div>
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
        ...
        <p className='text-lg font-medium text-white'> Chatting Made Eazy!!!</p>
    </div>
  )
}
